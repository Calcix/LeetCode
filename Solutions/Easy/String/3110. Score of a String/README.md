...
The given function calculates a "score" by summing the absolute differences between the ASCII values of consecutive characters in a string $s$.
...
class Solution:
    def scoreOfString(self, s: str) -> int:
        ...
        for i in range(len(s)-1):
            score += abs(ord(s[i])-ord(s[i+1])) #ord returns an integer representing the Unicode point 
        return score
```
